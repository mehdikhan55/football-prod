...
const Users = () => {
    ...
    const blockUser = (index) => {
        ...
        updatedUsers[index].status = updatedUsers[index].status === 'active' ? 'blocked' : 'active';
    ...
    return (
        <dialog id={modalId} className="modal">
            <div className="modal-box bg-gray-900 text-white w-11/12 max-w-5xl relative p-6 rounded-lg shadow-lg">
                <h3 className="font-bold text-2xl mb-4 border-b border-gray-700 pb-2">User Details: {user.username}</h3>

                <div className="grid grid-cols-2 gap-4 text-lg leading-7">
                    <p><strong>Email:</strong> <span className="ml-4">{user.email}</span></p>
                    <p><strong>Phone:</strong> <span className="ml-4">{user.phone}</span></p>
                    <p><strong>Address:</strong> <span className="ml-4">{user.address}</span></p>
                    <p><strong>Team:</strong> <span className="ml-4">{user.team}</span></p>
                    <p><strong>Date of Birth:</strong> <span className="ml-4">{user.dob.toDateString()}</span></p>
                    <p><strong>Points:</strong> <span className="ml-4">{user.points}</span></p>
                    <p><strong>Status:</strong> <span className="ml-4">{user.status}</span></p>
                </div>
